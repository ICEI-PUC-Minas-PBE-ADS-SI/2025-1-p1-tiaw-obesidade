...
if (botaoConcluir) {
    botaoConcluir.addEventListener('click', async (event) => {
        ...
        try {
            const verificaEmail = await fetch(`http://localhost:3000/usuarios?email=${encodeURIComponent(email)}`);
        ...
        try {
            ...
            if (response.ok) {
                ...
                window.location.href = '/ON Focus/ON Focus Login/index.html?cadastro=sucesso';
            } else {
